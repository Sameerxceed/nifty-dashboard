...
def build_brief_html(text):
    ...
    nl2 = "\n\n"
    nl1 = "\n"
    return out.replace(nl2,"</p><p>").replace(nl1," ")
...
# Pre-compute intraday block (avoids backslash-in-f-string issue on Python < 3.12)
_intra_text = intraday_text.replace("\n\n", "</p><p>").replace("\n", " ")
_intra_lbl  = data.get("session_label", "")
_acc_html   = build_accuracy_card(acc)
_alt_html   = build_pivot_alerts(alerts) if alerts else ""
if is_intraday:
    intraday_block = (
        '<div class="sec-lbl">Intraday Update</div>'
        '<div class="grid2">'
        f'<div class="analysis-box"><div style="font-size:10px;font-weight:700;color:var(--accent);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">📊 {_intra_lbl}</div>'
        f'<div class="brief-text"><p>{_intra_text}</p></div></div>'
        f'<div style="display:flex;flex-direction:column;gap:10px">{_acc_html}{_alt_html}</div>'
        '</div>'
    )
else:
    intraday_block = ""
